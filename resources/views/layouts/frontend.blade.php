...
<html lang="en">
    <head>
        <title>{{ $title ?? 'BTS STORE' }}</title>
        <!-- Required meta tags -->
        <!-- CSRF Token -->
        <meta name="csrf-token" content="{{ csrf_token() }}">
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
        <meta content="Toko Buket Menjual berbagai macam hijab, jaket, aksesoris dan pakaian islami pria dan wanita" name="description">
        <meta content="{{ $title ?? 'App Toko' }}" name="keywords">
        <meta property="og:locale" content="ID_id"/>
        <meta property="og:type" content="website"/>
        <meta property="og:image" content="{{ asset('assets/img/flower.png') }}">
        <!-- Bootstrap CSS -->
        <link rel="shortcut icon" href="{{ asset('assets/img/flower.png') }}">
